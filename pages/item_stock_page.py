...
class ItemStockPage:
    ...
    ITEM_CATEGORY = (
        By.XPATH,
        "//label[normalize-space()='Select Item Category']/following::select[1]"
    )

    ITEM = (
        By.XPATH,
        "//label[normalize-space()='Select Item']/following::select[1]"
    )

    SUPPLIER = (
        By.XPATH,
        "//label[contains(normalize-space(),'Select Supplier')]/following::select[1]"
    )

    STORE = (
        By.XPATH,
        "//label[contains(normalize-space(),'Select Store')]/following::select[1]"
    )
    ...
    DATE = (
        By.XPATH,
        "//input[@placeholder='dd-mm-yyyy' or @placeholder='Enter Date']"
    )
    ...
    def enter_date(self, value):
        self.enter_text(self.DATE, value)

    def enter_purchase_price(self, value):
        self.enter_text(self.PURCHASE_PRICE, value)
    ...
    def click_edit_for_exact_row(self, expected_values):
        ...
        raise AssertionError(
            f"Edit button was not found for exact item stock row. "
    ...
    def click_delete_for_exact_row(self, expected_values):
        ...
        raise AssertionError(
            f"Delete button was not found for exact item stock row. "
            f"Expected values: {expected_values}. "
            f"Actual table text: {table_text}"
        )
